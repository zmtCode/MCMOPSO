...
def pearson(x, y):
    t = x.corr(y, method='spearman')
    return t


def sort_xname_by_Pearson(X, y):
    pearson_Series = pd.Series()
    xname = X.columns.tolist()
    for i in xname:
        pearson_value = pearson(X[i], y)  # 计算X[i]这一列和y的MIC值
        pearson_Series[i] = pearson_value
    pearson_Series = pearson_Series.sort_values(ascending=False)  # 降序排序
    xname_sorted = pearson_Series.index.tolist()
    return xname_sorted



def de_irrelevant(X_original, y, x_proportion=0.1, stepLength=0.01, cycles=103):
    """
            判断多少百分比的特征个数最合适
...
if __name__ == "__main__":
    ...
    df_x = pd.read_excel(r'C:\Users\Administrator\Desktop\paper2回归任务\data\blogData_test\blogData_test8.xlsx')  # 内源性物质
    # X = df_x
    # # print(X)
    # y = df_y['y4']
    X = df_x.iloc[:, :-1]
    ...
    y = df_x['y']
    Xname = X.columns.tolist()
    xname_sorted = sort_xname_by_Pearson(X, y)
    # print(xname_sorted)
